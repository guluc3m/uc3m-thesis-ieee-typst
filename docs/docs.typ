...
#doc-table(
  ...
  [`array` of `str`],
  [-],
  [List of advisor names (at least one).],
  [`location`\*],
  [`str`],
  [-],
  [Presentation location (e.g. `"Leganés, Madrid"`).],
  [`thesis-type`\*],
  [`str`],
  [-],
  [`"TFG"` (Bachelor) or `"TFM"` (Master).],
  [`date`\*],
  [`datetime`],
  [-],
  [Presentation date.],
  ...
  [`dictionary`],
  [-],
  [Abstract content and keywords. See @sec:abstract.],
  [`language`\*],
  [`str`],
  [-],
  [`"en"` or `"es"`.],
  [`genai-declaration`\*],
  [`dictionary` or `content`],
  [-],
  [Generative AI usage declaration. See @sec:genai-declaration.],
  [`bibliography-content`],
  [`content`],
  [`none`],
  [Bibliography contents, typically the result of calling `bibliography(...)`.],
  [`style`],
  [`str`],
  [`"fancy"`],
  [Visual style: `"fancy"`, `"clean"`, or `"strict"`. See @sec:styles.],
  [`titlepage-style`],
  [`str` or `auto`],
  [`auto`],
  [Style for the title page. If `auto`, uses the main `style`.],
  [`table-style`],
  [`str` or `auto`],
  [`auto`],
  [Caption style for tables: `"ieee"` or `"clean"`. If `auto`, uses `"clean"` for the clean style and `"ieee"` for the others.],
  [`figure-style`],
  [`str` or `auto`],
  [`auto`],
  [Caption style for figures: `"ieee"` or `"clean"`. Defaults the same as `table-style`.],
  [`figure-spacing`],
  [`length` or `none`],
  [`0.75em`],
  [Extra vertical spacing around figures and tables. Set to `none` to disable.],
  [`double-sided`],
  [`bool`],
  [`false`],
  [Whether to use double-sided page layout. Not allowed with `"strict"` style.],
  [`logo`],
  [`str`],
  [`"new"`],
  [UC3M logo variant: `"new"` (horizontal color logo) or `"old"` (shield logo).],
  [`short-title`],
  [`str` or `content`],
  [`none`],
  [Shorter version of the title for the page header. Only applies when `double-sided` is `true`.],
  [`date-format`],
  [`str` or `auto`],
  [`auto`],
  [Date format using #link("https://typst.app/docs/reference/foundations/datetime/#format")[Typst's format syntax]. Defaults to the locale's format.],
  [`license`],
  [`bool`],
  [`true`],
  [Whether to include the CC BY-NC-ND 4.0 license notice on the title page.],
  [`flyleaf`],
  [`bool`],
  [`true`],
  ...
  [`dictionary` or `none`],
  [`none`],
  [An opening quote. See @sec:epigraph.],
  [`english-abstract`],
  [`dictionary` or `none`],
  [`none`],
  [English translation of the abstract. Required for Spanish works, invalid for English ones.],
  [`acknowledgements`],
  [`content` or `none`],
  [`none`],
  ...
  [`dictionary` or `none`],
  [`none`],
  [Extra outlines to include (figures, tables, listings, custom). See @sec:outlines.],
  [`abbreviations`],
  [`dictionary` of `str`, `content`, or `none`],
  [`none`],
  [Abbreviations and acronyms list. See @sec:abbreviations.],
  [`appendixes`],
  [`content` or `none`],
  [`none`],
  [Appendix content. A generative AI declaration appendix is always appended automatically.],
  [`glossary`],
  [`array`, `content`, or `none`],
  [`none`],
  [Glossary entries for `glossarium`, or custom `content`. See @sec:glossary.],
)
